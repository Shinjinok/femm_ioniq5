...
def run_multi_current_sweep():
    ...
    # 10A ~ 350A, 50A 간격 설정
    current_list = np.arange(0, 351, 50)
    #current_list = [50]
    theta_e_list = np.radians(np.arange(0, 360, 6))  # 0° ~ 360°, 15° 간격
    phase = 'C'  # A상 기준으로 전류 설정
    summary_records = []
    ...
    for ia_val in current_list:
        print(f"\n--- [현재 전류 조건: {ia_val}A] phase{phase} 해석 진행 중 ---")
        
        total_tasks = len(theta_e_list)
        num_processes = min(cpu_count(), total_tasks)
        task_chunks = np.array_split(theta_e_list, num_processes)
        
        worker_args = [(idx, list(chunk), base_fem_path, magnet_material_name, rotor_group_no, pole_pairs, ia_val, phase) 
        ...
        plot_inductance_results(df_current, ia_val=ia_val, output_image_path=f"inductance_{phase}_{int(ia_val)}A.png")
